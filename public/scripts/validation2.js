...
function checkr(mail) {
  let regex = new RegExp("[a-z0-9]+@[a-z]+.[a-z]{2,3}$");
  document.getElementById("email-exist-signin").classList.add("hidden");

  if (regex.test(mail)) {
    document
      .getElementById("email-alertregister-signin")
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState == 4) {
        ...
        if (!res.isNotPresent) {
          document
            .getElementById("email-exist-signin")
